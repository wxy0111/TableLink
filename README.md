...
http://localhost:3000/admin/daily-closing
...
日结对账 /admin/daily-closing
  店长/老板使用，查看当天营业额、实收、退款、退菜、未结订单、操作流水和打印任务。
```
...
```txt
老板    13800000000 / PIN 1111
后厨    13800000001 / PIN 2222
收银员  13800000002 / PIN 3333
服务员  13800000003 / PIN 4444
店长    13800000004 / PIN 5555
```
...
scripts/
  ...
  verify-ledger-flow.mjs 账本和报表口径验证脚本
...
```powershell
npm.cmd run verify:auth-closing-flow
```

该脚本会验证老板、收银、后厨登录，未登录访问被拒绝，后厨不能访问日结，老板能读取日结对账。
...
```powershell
npm.cmd run verify:ledger-flow
```

该脚本会验证下单写入销售账本、收款写入实收账本、退款写入退款账本，并确认日结报表增量和账本一致。

## 账本和状态机

第一版开始把“钱”和“流程状态”从普通订单字段里拆出来管理：
...
报表口径以 `ledger_entries` 为准，订单金额和支付状态作为业务快照使用。这样以后加微信/支付宝回调、部分退款、反结账、跨日报表时，不需要反复从订单当前状态倒推历史金额。

状态流转统一放在后端状态机里校验，覆盖订单、菜品、支付和桌台。核心目标是避免出现“已退款又继续收款”“已清台还有未结订单”“已上桌菜品回到制作中”这类对账和前厅流程问题。
